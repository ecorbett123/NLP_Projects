"""
Natural Language Processing - Parsing with Probabilistic Context Free Grammars
Fall 2023 - Emma Corbett
"""
import math
import sys
...
class CkyParser(object):
                    ...
                    if (i, k) in parse_table and (k, j) in parse_table:
                        ...
                        for a_nonterminal in a_nonterminals:
                            for b_nonterminal in b_nonterminals:
                                if (a_nonterminal[0], b_nonterminal[0]) in self.grammar.rhs_to_rules:
    ...
    def parse_with_backpointers(self, tokens):
        ...
        # initialization
        for i in range(num_tokens):
            # Comment this out to preserve formatting...
            # if (tokens[i],) not in self.grammar.rhs_to_rules:
            ...
            for i in range(0, num_tokens - leng + 1):
                ...
                for k in range(i + 1, j):
                    ...
                    if (i, k) in table and (k, j) in table:
                        ...
                        for a_nonterminal in a_nonterminals:
                            for b_nonterminal in b_nonterminals:
                                if (a_nonterminal, b_nonterminal) in self.grammar.rhs_to_rules:
                                    # for each of the rules, add them to map of greater probability
                                    for nonterminal in self.grammar.rhs_to_rules[(a_nonterminal, b_nonterminal)]:
...
def get_tree(chart, i,j,nt): 
    ...
    if (i,j) not in chart or nt not in chart[(i,j)]:
        raise KeyError # throw a key error to get correct coverage score (happens when index not in char or token not in index of char)
    if isinstance(chart[(i,j)][nt], str):
        return (nt, chart[(i,j)][nt])

    split1 = chart[(i,j)][nt][0]
    split2 = chart[(i, j)][nt][1]
    left = get_tree(chart, split1[1], split1[2], split1[0])
    right = get_tree(chart, split2[1], split2[2], split2[0])
